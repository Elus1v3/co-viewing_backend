...
func (s *Store) GetFriendList(ctx context.Context, currentUserId int) ([]models.User, error) {
	sqlQuery := `
        SELECT u.id_pk, u.nickname
        FROM user_friends uf
        JOIN "user" u 
          ON u.id_pk = CASE
              WHEN uf.user_id_fk = $1 THEN uf.friend_id_fk
              ELSE uf.user_id_fk
          END
        WHERE (uf.user_id_fk = $1 OR uf.friend_id_fk = $1)
          AND uf.status = 'accepted';
    `

	var friends []models.User
	rows, err := s.conn.Query(ctx, sqlQuery, currentUserId)
	if err != nil {
		slog.Error("failed get friens", "error", err)
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) DeleteFriendRequest(ctx context.Context, request models.FriendRequest) error {
	sqlQuery := `
		DELETE FROM user_friends 
		WHERE (user_id_fk = $1 AND friend_id_fk = $2)
   		OR (user_id_fk = $2 AND friend_id_fk = $1);
	`

	commandTag, err := s.conn.Exec(ctx, sqlQuery, request.UserId, request.FriendId)
	if err != nil {
		slog.Error("failed delete friend request", "error", err)
		return err
	}
	if commandTag.RowsAffected() != 1 {
		slog.Error("failed delete friend request")
		return errors.New("failed delete friend request")
	}
